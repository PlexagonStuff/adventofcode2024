        ...
        try (BufferedReader fileReader = Files.newBufferedReader(Paths.get("input4.txt"))) {
            ...
            for (int y = 0; y < wordSearch.size(); y++) {
                for (int x = 0; x < wordSearch.get(y).size(); x++) {
                    if (wordSearch.get(y).get(x) == 'X') {
                        totalXMASFound += findAbove(y, x);
                        totalXMASFound += findBelow(y, x);
                        totalXMASFound += findForwards(y, x);
                        totalXMASFound += findBackwards(y, x);
                        totalXMASFound += findNE(y, x);
                        totalXMASFound += findNW(y, x);
                        totalXMASFound += findSE(y, x);
                        totalXMASFound += findSW(y, x);
                    }
                }
            }
